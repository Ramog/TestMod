...
public class ContainerPedestal extends Container{
	...
	public ContainerPedestal(InventoryPlayer playerInv, final TileEntityPedestal pedestal) {
		...
		addSlotToContainer(new SlotItemHandler(inventory, 0, 80, 35) { 		//Slot one change max slots in tileentity Itemstackhandler
			@Override														//one slot is 18px wide the inventory is 160 px wide
			public void onSlotChanged() {
				pedestal.markDirty();
			}
		});		
		
		//Dummy playerInventory since it isn't normaly in the new screen
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 9; j++) {
